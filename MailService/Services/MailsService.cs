...
namespace MailService.Services
{
        public class MailsService   //deals with sending emails
        {
            ...
            public async Task sendEmail(UserMessageDto user, string Message, string? subject = "Welcome to AuctionGarde") //q
            {
                MimeMessage message1 = new MimeMessage();

                message1.From.Add(new MailboxAddress("View auctions ", _email));  //where email is coming from
